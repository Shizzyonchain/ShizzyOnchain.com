...
import type { FxPoint } from "../../lib/currency-returns";
...
export async function GET() {
  ...
  if (!points.size) return NextResponse.json({ error: "TAO dollar history unavailable" }, { status: 503 });
  return NextResponse.json({ data: [...points.values()].sort((a, b) => a.time - b.time), source: "coinbase", resolution: "1m" }, {
