...
export async function GET() {
  const feeds = [
    async () => {
      const response = await fetch("https://api.coinbase.com/v2/prices/TAO-USD/spot", { cache: "no-store" });
    ...
    async () => {
      const response = await fetch("https://api.coinpaprika.com/v1/tickers/tao-bittensor", { cache: "no-store" });
      if (!response.ok) throw new Error();
      const data = await response.json();
