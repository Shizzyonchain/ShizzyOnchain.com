  ...
  return <main className="shell">
    ...
    {view === "screener" ? <>
      ...
      <section className="screener panel">
        ...
        <div className="table-wrap"><table><thead><tr><th>#</th><th><button onClick={()=>changeSort("netuid")}>Subnet{sortArrow("netuid")}</button></th><th><button onClick={()=>changeSort("price_tao")}>Price {currency === "usd" ? "$" : "τ"}{sortArrow("price_tao")}</button></th><th><button onClick={()=>changeSort("change_10m")}>10 Minutes{sortArrow("change_10m")}</button></th><th><button onClick={()=>changeSort("change_1h")}>1 Hour{sortArrow("change_1h")}</button></th><th><button onClick={()=>changeSort("change_24h")}>1 Day{sortArrow("change_24h")}</button></th><th><button onClick={()=>changeSort("emission_pct")}>Emission %{sortArrow("emission_pct")}</button></th><th title="Estimated compounded yield for alpha stakers using the current validator allocation and Root Proportion, before validator take and price changes."><button onClick={()=>changeSort("apy")}>Est. Staker APY{sortArrow("apy")}</button></th><th title="Total conviction-locked alpha, valued at the current subnet price."><button onClick={()=>changeSort("conviction_locked_tao")}>Conviction Locked{sortArrow("conviction_locked_tao")}</button></th><th><button onClick={()=>changeSort("volume_24h_tao")}>Volume{sortArrow("volume_24h_tao")}</button></th><th><button onClick={()=>changeSort("tao_reserve")}>Liquidity{sortArrow("tao_reserve")}</button></th><th><button onClick={()=>changeSort("market_cap_tao")}>Mkt cap{sortArrow("market_cap_tao")}</button></th></tr></thead>
        <tbody>{filtered.map((r,i)=><tr key={r.netuid} className={r.netuid===selected?"selected":""} onClick={()=>setSelected(r.netuid)}><td>{i+1}</td><td><span className="token">{r.symbol?.replace("α","") || r.netuid}</span><div><b>{r.name || `Subnet ${r.netuid}`}</b><small>SN{r.netuid}</small></div></td><td>{money(r.price_tao, true)}</td>{[r.change_10m,r.change_1h,r.change_24h].map((v,j)=><td key={j} className={changeClass(v)}>{Number(v||0)>0?"+":""}{fmt(v)}%</td>)}<td className="emission-cell">{r.emission_pct == null ? "—" : `${fmt(r.emission_pct, 4)}%`}<small>of each block</small></td><td className="apy-cell" title="Estimated compounded yield for alpha stakers using the current validator allocation and Root Proportion, before validator take and price changes.">{r.apy == null ? "—" : `${fmt(r.apy, Number(r.apy) < 1 ? 4 : 2)}%`}<small>0% validator take</small></td><td>{r.conviction_locked_tao == null ? "—" : money(r.conviction_locked_tao)}<small>{r.conviction_locked_alpha == null ? "" : `${fmt(r.conviction_locked_alpha, 2)} α locked`}</small></td><td>{money(r.volume_24h_tao)}</td><td>{money(r.tao_reserve)}</td><td>{money(r.market_cap_tao)}</td></tr>)}</tbody></table></div>
      </section>
