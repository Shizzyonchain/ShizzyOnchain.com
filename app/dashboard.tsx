...
const demoRows: ScreenerRow[] = [
  { netuid: 64, name: "Chutes", symbol: "Î±64", price_tao: "0.1842", market_cap_tao: "246812", tao_reserve: "18842", volume_24h_tao: "3284", change_1h: "1.84", change_24h: "8.42", change_7d: "24.8" },
  { netuid: 4, name: "Targon", symbol: "Î±4", price_tao: "0.0541", market_cap_tao: "184402", tao_reserve: "12770", volume_24h_tao: "2241", change_1h: "-0.74", change_24h: "3.18", change_7d: "12.4" },
  { netuid: 18, name: "Cortex.t", symbol: "Î±18", price_tao: "0.0328", market_cap_tao: "115092", tao_reserve: "9414", volume_24h_tao: "1628", change_1h: "2.14", change_24h: "-1.92", change_7d: "8.7" },
  { netuid: 8, name: "Proprietary", symbol: "Î±8", price_tao: "0.0194", market_cap_tao: "88430", tao_reserve: "7218", volume_24h_tao: "1102", change_1h: "0.38", change_24h: "6.51", change_7d: "-4.2" },
  { netuid: 1, name: "Apex", symbol: "Î±1", price_tao: "0.0147", market_cap_tao: "74611", tao_reserve: "6390", volume_24h_tao: "942", change_1h: "-1.12", change_24h: "-4.77", change_7d: "18.1" },
];
...
export function Dashboard() {
  ...
  const money = (value?: string | number, price = false) => {
    ...
    if (currency === "tao") return `Ï„ ${fmt(tao, price ? 6 : 4)}`;
    if (!taoUsd) return "$â€”";
  ...
  return <main className="shell">
    ...
    {view === "screener" ? <>
      ...
      <section className="screener panel">
        <div className="screener-head"><div><p className="eyebrow">Bittensor markets</p><h2>Subnet screener</h2></div><label className="search"><span>âŒ•</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search subnet or netuid" /></label></div>
        <div className="table-wrap"><table><thead><tr><th>#</th><th>Subnet</th><th><button onClick={()=>setSort("price_tao")}>Price {currency === "usd" ? "$" : "Ï„"}</button></th><th><button onClick={()=>setSort("change_1h")}>1H</button></th><th><button onClick={()=>setSort("change_24h")}>24H</button></th><th><button onClick={()=>setSort("change_7d")}>7D</button></th><th><button onClick={()=>setSort("volume_24h_tao")}>Volume</button></th><th><button onClick={()=>setSort("tao_reserve")}>Liquidity</button></th><th><button onClick={()=>setSort("market_cap_tao")}>Mkt cap</button></th></tr></thead>
        <tbody>{filtered.map((r,i)=><tr key={r.netuid} className={r.netuid===selected?"selected":""} onClick={()=>setSelected(r.netuid)}><td>{i+1}</td><td><span className="token">{r.symbol?.replace("Î±","") || r.netuid}</span><div><b>{r.name || `Subnet ${r.netuid}`}</b><small>SN{r.netuid}</small></div></td><td>{money(r.price_tao, true)}</td>{[r.change_1h,r.change_24h,r.change_7d].map((v,j)=><td key={j} className={changeClass(v)}>{Number(v||0)>=0?"+":""}{fmt(v)}%</td>)}<td>{money(r.volume_24h_tao)}</td><td>{money(r.tao_reserve)}</td><td>{money(r.market_cap_tao)}</td></tr>)}</tbody></table></div>
      </section>
    </> : <section className="wallet-page">
      <div className="wallet-intro"><p className="eyebrow">Portfolio intelligence</p><h1>See every wallet.<br/><span>See the whole position.</span></h1><p>Paste up to 100 Bittensor coldkeys. Weâ€™ll combine free TAO, alpha positions, subnet exposure, and spot-value estimates at one finalized block.</p></div>
      <form className="wallet-form panel" onSubmit={checkWallets}><label htmlFor="wallets">Coldkey addresses</label><textarea id="wallets" value={walletInput} onChange={e=>setWalletInput(e.target.value)} placeholder={"5F...\n5G...\n5H..."} /><div className="form-foot"><span>One per line, space, or comma</span><button disabled={checking}>{checking ? "Checking chainâ€¦" : "Check wallets â†’"}</button></div>{walletError && <p className="form-error">{walletError}</p>}</form>
      ...
      <div className="wallet-results">{wallets.map(w=><article className="wallet-card panel" key={w.address}><div className="wallet-card-head"><span className="wallet-ident">{w.address.slice(0,6)}</span><div><b>{w.address.slice(0,12)}â€¦{w.address.slice(-8)}</b><small>{w.stakes.length} positions</small></div><strong>{money(w.total_tao_value)}</strong></div><div className="wallet-split"><span>Free <b>{money(w.free_tao)}</b></span><span>Staked <b>{money(w.staked_tao_value)}</b></span></div><div className="positions">{w.stakes.slice(0,8).map((s,i)=><div key={`${s.hotkey}-${s.netuid}-${i}`}><span>SN{s.netuid}<small>{s.hotkey.slice(0,7)}â€¦</small></span><span>{fmt(s.alpha,4)} Î±<small>{money(s.tao_value)}</small></span></div>)}</div></article>)}</div>
      {!wallets.length && <div className="wallet-empty"><div className="radar"><i/><i/><i/></div><p>Your combined portfolio will appear here.</p></div>}
    </section>}
    <footer><span>SHIZZYUNCHAINED</span><p>Finalized on-chain data Â· {currency === "usd" ? "USD values use the live TAO spot rate" : "TAO-denominated values"} Â· Not financial advice</p><b>Built on Bittensor</b></footer>
  </main>;
}
