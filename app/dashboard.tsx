    ...
    {view === "screener" ? <>
      ...
      <section className="chain-activity panel" aria-labelledby="activity-title">
        ...
        <div className="activity-toolbar"><div role="group" aria-label="Filter chain events">{[
          ["all", "All activity"], ["locks", "Conviction"], ["keys", "Keys & owners"],
      ...
      {wallets.length > 0 && walletDisplay === "portfolio" && <section className="portfolio-view panel" role="tabpanel"><div className="portfolio-chart-column"><div className="portfolio-chart-head"><div><p className="eyebrow">Portfolio allocation</p><span>Grouped across every imported wallet</span></div><div className="portfolio-metric" aria-label="Portfolio display"><button className={portfolioMetric === "allocation" ? "active" : ""} onClick={() => setPortfolioMetric("allocation")}>Allocation %</button><button className={portfolioMetric === "value" ? "active" : ""} onClick={() => setPortfolioMetric("value")}>Value</button></div></div><div className="portfolio-donut" style={{ "--portfolio-gradient": portfolioGradient } as CSSProperties} role="img" aria-label={`Portfolio allocation across ${portfolioAssets.length} assets`}><div><span>{hideBalances ? "Balance hidden" : "Combined value"}</span><strong>{privateMoney(portfolioTotal)}</strong><small>{portfolioAssets.length} assets</small></div></div><div className="portfolio-legend">{portfolioChartAssets.map((asset,index)=>{const allocation=portfolioTotal ? asset.taoValue / portfolioTotal * 100 : 0;return <div key={asset.key}><em>{String(index + 1).padStart(2,"0")}</em><i style={{ background: portfolioColors[index % portfolioColors.length] }}/><span><b>{asset.netuid == null ? asset.symbol : `SN${asset.netuid}`} <small>· {asset.name}</small></b><span className="portfolio-mini-bar"><i style={{width:`${Math.max(1,allocation)}%`,background:portfolioColors[index % portfolioColors.length]}}/></span></span><strong>{portfolioMetric === "allocation" ? `${allocation.toFixed(1)}%` : privateMoney(asset.taoValue)}</strong></div>})}</div></div><div className="portfolio-assets"><div className="portfolio-assets-head"><div><p className="eyebrow">Combined allocation</p><h2>All assets</h2></div><span>Grouped across {wallets.length} wallets</span></div>{portfolioAssets.map((asset,index)=><article key={asset.key}><div><i className="portfolio-color" style={{ background: portfolioColors[index % portfolioColors.length] }}/><div><b>{asset.name}</b><small>{asset.netuid == null ? asset.symbol : `SN${asset.netuid}`} · {asset.wallets.size} {asset.wallets.size === 1 ? "wallet" : "wallets"}</small></div></div><div className="portfolio-bar"><i style={{ width: `${portfolioTotal ? Math.max(1, asset.taoValue / portfolioTotal * 100) : 0}%`, background: portfolioColors[index % portfolioColors.length] }}/></div><div><strong>{privateMoney(asset.taoValue)}</strong><small>{portfolioTotal ? (asset.taoValue / portfolioTotal * 100).toFixed(2) : "0.00"}%{asset.netuid != null ? ` · ${hideBalances ? "•••• α" : `${fmt(asset.alpha,4)} α`}` : ""}</small></div></article>)}</div></section>}
