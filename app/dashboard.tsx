...
export function Dashboard({ initialView = "screener" }: { initialView?: DashboardView }) {
  ...
  return <main className="shell">
    ...
    {view === "screener" ? <>
      ...
      {wallets.length > 0 && walletDisplay === "portfolio" && <section className="portfolio-view portfolio-snapshot panel" role="tabpanel"><div className="portfolio-chart-head"><div><p className="eyebrow">Portfolio allocation</p><span>{portfolioAssets.length} assets grouped across {wallets.length} wallets</span></div><div className="portfolio-metric" aria-label="Portfolio display"><button className={portfolioMetric === "allocation" ? "active" : ""} onClick={() => setPortfolioMetric("allocation")}>Allocation %</button><button className={portfolioMetric === "value" ? "active" : ""} onClick={() => setPortfolioMetric("value")}>Value</button></div></div><div className="portfolio-snapshot-body"><div className="portfolio-donut" style={{ "--portfolio-gradient": portfolioGradient } as CSSProperties} role="img" aria-label={`Portfolio allocation across ${portfolioAssets.length} assets`}><div><span>{hideBalances ? "Balance hidden" : "Combined value"}</span><strong>{privateMoney(portfolioTotal)}</strong><small>{portfolioAssets.length} assets</small></div></div><div className="portfolio-legend">{portfolioChartAssets.map((asset,index)=>{const allocation=portfolioTotal ? asset.taoValue / portfolioTotal * 100 : 0;return <div key={asset.key}><em>{String(index + 1).padStart(2,"0")}</em><i style={{ background: portfolioColors[index % portfolioColors.length] }}/><span><b>{asset.netuid == null ? asset.symbol : `SN${asset.netuid}`} <small>· {asset.name}</small></b><span className="portfolio-mini-bar"><i style={{width:`${Math.max(1,allocation)}%`,background:portfolioColors[index % portfolioColors.length]}}/></span></span><strong>{portfolioMetric === "allocation" ? `${allocation.toFixed(1)}%` : privateMoney(asset.taoValue)}</strong></div>})}</div></div></section>}
