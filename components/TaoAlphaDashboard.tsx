...
export const TaoAlphaDashboard: React.FC = () => {
  ...
  const SortHeader = ({ label, field }: { label: string, field: SortField }) => (
    <th className="px-6 py-5 cursor-pointer group" onClick={() => handleSort(field)}>
      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 group-hover:text-blue-500 transition-colors">
        {label}
        <ArrowUpDown size={12} className={`opacity-0 group-hover:opacity-100 transition-opacity ${sortField === field ? 'opacity-100 text-blue-500' : ''}`} />
      </div>
    </th>
  );
    ...
    <div className="max-w-[1400px] mx-auto px-6 py-10 space-y-12 animate-in fade-in duration-700">
      ...
      {loading && subnets.length === 0 ? (
        ...
        <div className="h-96 flex flex-col items-center justify-center gap-6 p-10 border border-dashed border-rose-500/20 rounded-[3rem]">
          ...
          <button onClick={fetchSubnets} className="text-blue-500 font-black uppercase tracking-widest text-[10px] hover:underline">Retry UPLINK</button>
        </div>
      ) : (
            ...
            <table className="w-full text-left border-collapse min-w-[1000px]">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                {sortedSubnets.map((subnet) => (
                  <tr key={subnet.netuid} className="group hover:bg-blue-600/[0.03] transition-colors">
                    <td className="px-6 py-6 font-mono font-black text-blue-600 dark:text-blue-400 text-lg italic">
                    ...
                    <td className="px-6 py-6 text-center">
                      <a 
                        ...
                        className="inline-flex p-3 rounded-xl border border-slate-200 dark:border-white/10 text-slate-400 hover:text-blue-600 hover:border-blue-500/50 transition-all"
                      >
