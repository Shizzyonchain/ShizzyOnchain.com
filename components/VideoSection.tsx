...
export const VideoSection: React.FC<VideoSectionProps> = ({ title, videos, aspectRatio, onViewAll, limit }) => {
  ...
  return (
    <section className="mt-12">
      ...
      {videos.length === 0 ? (
        <div className="w-full py-20 border border-dashed border-slate-200 dark:border-white/10 rounded-3xl flex flex-col items-center justify-center gap-4 bg-white/50 dark:bg-white/[0.01]">
          <VideoOff size={40} className="text-slate-300 dark:text-slate-700" />
          <p className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-[0.3em]">No Content Sync'd Yet</p>
          <p className="text-[9px] font-mono text-slate-400/60 uppercase tracking-widest">Awaiting Manual Ingress or YouTube Link</p>
        </div>
      ) : (
