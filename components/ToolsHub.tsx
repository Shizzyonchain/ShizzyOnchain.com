...
const ToolCard: React.FC<ToolCardProps> = ({ title, description, icon, view, colorClass, onNavigate }) => (
  <button 
    onClick={() => onNavigate(view)}
    className="group relative flex flex-col bg-white dark:bg-[#0b0e14] border border-slate-200 dark:border-white/5 rounded-[2.5rem] p-8 md:p-10 transition-all duration-500 hover:border-blue-500/40 hover:shadow-2xl hover:shadow-blue-500/5 text-left overflow-hidden"
  >
    <div className={`absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity ${colorClass}`}>
      {icon}
    </div>
    
    <div className={`p-4 rounded-2xl mb-8 w-fit ${colorClass} bg-opacity-10 dark:bg-opacity-10 border border-current border-opacity-20`}>
...
export const ToolsHub: React.FC<{ onNavigate: (view: View) => void }> = ({ onNavigate }) => {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 py-12 space-y-20 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row items-end justify-between gap-8 border-b border-slate-200 dark:border-white/10 pb-16">
        ...
        <div className="hidden lg:block">
           <div className="flex items-center gap-6 opacity-40">
              <div className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest">Latency: 24ms</div>
