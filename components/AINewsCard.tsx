...
export const AINewsCard: React.FC<AINewsCardProps> = ({ item }) => {
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
    // Fallback to a guaranteed high-quality generic AI image if the provided link fails
  ...
  return (
    <div className="relative group flex flex-col h-full">
      <a 
        ...
        className="flex flex-col h-full bg-white dark:bg-[#1e293b]/20 rounded-[2.5rem] overflow-hidden border border-slate-200 dark:border-white/5 hover:border-blue-500/50 transition-all duration-500 shadow-sm hover:shadow-2xl dark:hover:bg-white/[0.04] active:scale-[0.98]"
      >
        <div className="relative aspect-[16/10] overflow-hidden bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-white/5">
          ...
          <div className="absolute top-5 left-5">
            <span className="bg-black/70 backdrop-blur-xl text-white text-[9px] font-black px-4 py-2 rounded-xl uppercase tracking-[0.2em] font-mono border border-white/10 shadow-lg">
              {item.source}
            </span>
          </div>
        </div>
        ...
        <div className="p-8 flex flex-col flex-grow bg-gradient-to-b from-transparent to-slate-50/30 dark:to-white/[0.01]">
          ...
          <h3 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white leading-[1.1] mb-4 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors font-space italic uppercase tracking-tight">
            {item.title}
          </h3>
          ...
          <div className="flex items-center justify-between pt-6 border-t border-slate-200 dark:border-white/5">
            <span className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-[0.3em] font-mono inline-flex items-center gap-2 group-hover:translate-x-1 transition-transform">
              READ FULL INTEL <ExternalLink size={14} />
            </span>
          </div>
        </div>
      </a>
